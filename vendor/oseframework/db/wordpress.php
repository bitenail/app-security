	...
	public static function getConnection()
	{
		if(!isset(self::$dbh) && empty(self::$dbh)) {
			...
			if (!strstr($config->host, 'sock')) {
				...
				$dsn = 'mysql:unix_socket='. $host [1].';dbname=' . $config->db;
			}
			try {
	...
	public function getTableList()
	{
		...
		foreach ($results as $result)
		{
            if (preg_match($pattern, $result[0])) {
                if (!preg_match("/(ose_app_geoip)|(osefirewall_country)|(wp_osefirewall_vshash)|(osefirewall_vspatterns)|(osefirewall_basicrules)/", $result[0])) {
                    $list[] = $result[0];
                }
            }
		}
