...
class CronjobsController extends \App\Base
{
    public function action_saveCronConfig()
    {
        ...
        $clearbackuptime = $this->model->getVar('clearbackuptime', null);
        ...
        if ((!isset($custhours) AND $custhours != '') || (empty($custweekdays) )) {
            ...
            if (!empty($clearbackuptime)) {
                $type = 'clearbackup';
                $data['clearbackuptime'] = $clearbackuptime;
                $this->model->saveConfigurationNoExit($type, $data);
            }
            $custweekdays = base64_encode($this->model->JSON_encode($custweekdays));
            $result = $this->model->saveCronConfig($custhours, $custweekdays, $schedule_type, $cloudbackuptype, $enabled, $gitbackupfrequency);
        }
    }
