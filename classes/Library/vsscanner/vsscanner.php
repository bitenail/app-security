...
//		ini_set("display_errors", "off");
	...
	protected function saveFilesFromPath($scanPath)
	{
		...
		foreach ($objects as $path => $dir) {
			if (is_file($path) && !strstr($path, '.git/') && !strstr($path, OSE_FWDATA.ODS.'vsscanPath') && substr($path, -9) != '.svn-base') {
				$extension = substr($path, -4);
					if (!in_array($extension, array('.pbk', '.png', '.pdf', '.zip', '.jpa', '.ttf', '.svg', '.otf', '.eot'))) {
...
//		}
	...
	private function getCrawbackURL($key, $con, $process, $size, $confirm, $remainingFiles = array())
	{
		...
		if (!empty($remainingFiles)) {
			return "https://api2.centrora.com/vsscan/setupCronBatches?webkey=" . $webkey . "&key=" . $key . "&totalScanNum=" . $remainingFiles['totalScanNum'];
		}
		else {
			if ($con == false) {
				$status = $this->getWebsiteStatus();
				return "https://api2.centrora.com/vsscan/completeVSScanV6?webkey=" . $webkey . "&key=" . $key . "&con=" . $con . "&status=" . (int)$status . "&confirm=" . (int)$confirm;
			} else {
				return "https://api2.centrora.com/vsscan/scanresultsCallback?webkey=" . $webkey . "&key=" . $key . "&con=" . $con . "&process=" . $process;
			}
		}
	}
	...
	public function bgscan()
	{
		$webkey = $this->getWebKey();
		if (!empty($webkey)) {
			$url = "https://api2.centrora.com/vsscan/bgscan?webkey=" . $webkey;
			$this->sendRequestVS($url);
