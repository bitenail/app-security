...
class surfScanner
{
    ...
    {
        $data = array();
        $url = 'https://www.centrora.com/accountApi/Maldetect/checkLastMD5Update';
        ...
        if (empty($localresult)) {
            $data['msg'] = "Your MD5 Hash Database is not up to date.";
            $data['status'] = false;
            return $data;
        } else {
            $flag = $serverresult == $localresult ? " MD5 Hash is up to date. " : " MD5 Hash is available to update. ";
            $data['msg'] = $flag . 'Last update date: ' . $localresult;
            $data['status'] = true;
            return $data;
        }
    }
    ...
    {
        $url = 'https://www.centrora.com/accountApi/Maldetect/getUpdateMD5Data';
        $resultlist = oseJSON::decode( $this->getJsonData($url), true );
        $this->deleteMD5DBData();
        $this->insertMD5DBData($resultlist);
        return true;
    }
