			...
			if ($uri == 'SERVER')
			{
				...
				if (!empty($_SERVER['PHP_SELF']) && !empty($_SERVER['REQUEST_URI']))
				{
					...
					if (!empty($_SERVER['SERVER_PORT']) && $_SERVER['SERVER_PORT'] !=80) {
						$theURI = 'http' . $https . $_SERVER['HTTP_HOST'] .':'.$_SERVER['SERVER_PORT'] .  $_SERVER['REQUEST_URI'];
					}
					else {
				...
				else
				{
					...
					if (!empty($_SERVER['PORT']) && $_SERVER['SERVER_PORT'] !=80) {
						$theURI = 'http' . $https . $_SERVER['HTTP_HOST'].':'.$_SERVER['SERVER_PORT'] . $_SERVER['SCRIPT_NAME'];
					}
					else
					{
