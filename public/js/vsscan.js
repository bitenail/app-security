...
function downloadRequest(type) {
    jQuery(document).ready(function ($) {
        $.ajax({
            ...
            success: function (data) {
                // add virus signature animatiomn
                $('#icon-refresh').removeClass('spinAnimation');
                //$('#v-sig').text(data.result);
                $("#vs-div-update").hide();
...
function checkVirusSignatureVersion() {
    jQuery(document).ready(function ($) {
        $.ajax({
            type: "GET",
            url: url,
            ...
            data: {
                option: option,
                controller: controller,
                action: 'checkVirusSignatureVersion',
                task: 'checkVirusSignatureVersion',
                centnounce: $('#centnounce').val()
            },
            success: function (data) {
                if (data.status == true) {
                    $('#icon-refresh').addClass('spinAnimation');
                    $('#v-sig').text('Checking virus signature...');
                    downloadRequest('avs');
                } else {
                    $("#vs-div-update").hide();
                    $("#vs-div-uptodate").show();
...
function updateVirusSignatureVersionFile() {
    jQuery(document).ready(function ($) {
        $.ajax({
            type: "GET",
            url: url,
            ...
            data: {
                option: option,
                controller: controller,
                action: 'updateVirusSignatureVersionFile',
                task: 'updateVirusSignatureVersionFile',
                centnounce: $('#centnounce').val()
            },
            success: function (data) {
                if (data.status == true) {
                    //alert("virus signature file has been updated");
                    $("#vs-div-update").hide();
                    $("#vs-div-uptodate").show();
                }
                else {
                    //alert("there was some problems in updating the LastVersionCheck check file");
                    location.reload();
                }
            }
        });
...
jQuery(document).ready(function ($) {
    $.ajax({
        ...
        success: function (data) {
            if (data) {
                $('#vs-uptodate').text(data.year+ "/" +data.month+ "/"+data.date+ "  "+data.hour+ ":"+data.min+ ":"+data.sec);
                //alert(data.year+ "/" +data.month+ "/"+data.date+ "  "+data.hour+ ":"+data.min+ ":"+data.sec);
            }
            else {
                //alert("there was some problems in getting the date and time from the local file");
            }
        }
    });
});
